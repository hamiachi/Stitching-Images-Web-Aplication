...
void Util::stitch(std::vector<Mat> images, std::vector<Mat> homographies, int width, int height) {
    ...
    for (int i = 0; i < pano_mask.rows; ++i) {
        
        // #pragma omp parallel for

        for (int j = 0; j < pano_mask.cols*3; ++j) {
            if (pano_mask.at<float>(i,j) != 0.0 && img_mask.at<float>(i,j) != 0.0) {
                    pano_mask.at<float>(i,j) = ((float)pano_mask.at<float>(i,j) + (float)img_mask.at<float>(i,j))/2;
             }
             else {
    ...
    //displayImg(panorama);
    imwrite("/home/ntduc/Downloads/result.jpg", pano_mask * 255);
}
